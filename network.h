...
static __always_inline void swap_ip_addresses(struct __sk_buff *skb) {
  ...
  bpf_skb_store_bytes(skb, IP_SRC_OFF, dst_ip, 4, BPF_F_RECOMPUTE_CSUM);
  bpf_skb_store_bytes(skb, IP_DST_OFF, src_ip, 4, BPF_F_RECOMPUTE_CSUM);

  //__u32 *src_ip_ptr = (__u32 *)src_ip;
  //__u32 *dst_ip_ptr = (__u32 *)dst_ip;
  //bpf_l3_csum_replace(skb, IP_CSUM_OFF, *dst_ip_ptr, *src_ip_ptr, 4);
}
